...
SYSTEM_PRINCIPLES_TEXT_FILE = 'prompts/system_3.txt'
QUESTION_TEXT_FILE = 'prompts/question_3.txt'
CRITERIA = 'prompts/principle_3.txt'
...
def evaluater(system_prompt, question):    
    ...
    gpt_reason = get_completion(request_claude_magic(system_prompt,messages))
    #print(gpt_answer)
...
df = pd.read_csv('gold_50_3.csv')
df['questionID'] = df['questionID'].astype(str)
df['auto_criteria_3'] = '100'
...
for file_name in os.listdir(in_directory):
    ...
    if os.path.isfile(file_path):
        if file_name in os.listdir(out_directory):
            ...
            df.loc[df['questionID']==file_name.split('.')[0],'auto_criteria_3'] = mcq_eval[-1]['content']
        else:
            ...
            df.loc[df['questionID']==file_name.split('.')[0],'auto_criteria_3'] = mcq_eval
